...
def parse_lcov(lines):
  ...
  while len(lines):
    ...
    if profile.sf in files:
      old = files[profile.sf]
      files[profile.sf] = FileProfile(
        sf=profile.sf,
        fn=or_dict(old.fn, profile.fn),
        fnda=add_dict(old.fnda, profile.fnda),
        da=add_dict(old.da, profile.da),
      )
    else:
...
def iter_lcov_paths(lcov_files_path):
  with open(lcov_files_path) as files:
    ...
    files = [p for p in files if p.name == 'coverage.dat' and p.exists()]
